...
class Model5y4m_5y7m{
  ...
  static int CalculateSumOfVerbal(){
    int SumOfVerbal=0;
    SumOfVerbal+=Weigheddegrees5y4m_5y7m.understand(CalculationFromTester.understandDegree);
    // print("SumOfVerbal${SumOfVerbal}");
    SumOfVerbal+=Weigheddegrees5y4m_5y7m.information(CalculationFromTester.InformationDegree);
    // print("SumOfVerbal${SumOfVerbal}");
    SumOfVerbal+=Weigheddegrees5y4m_5y7m.calculation(CalculationFromTester.CalculationDegree);
    //  print("SumOfVerbal${SumOfVerbal}");
    SumOfVerbal+=Weigheddegrees5y4m_5y7m.similarity(CalculationFromTester.SimilarDegree);
    //  print("SumOfVerbal${SumOfVerbal}");
    SumOfVerbal+=Weigheddegrees5y4m_5y7m.vocublary(CalculationFromTester.VocublaryDegree);
    //  print("SumOfVerbal${SumOfVerbal}");
    SumOfVerbal+=Weigheddegrees5y4m_5y7m.repeatnumber(CalculationFromApp.RepeatNumber);
    // print("SumOfVerbal${SumOfVerbal}");
  ...
  static int CalculateSumOfpractical(){
    ...
    SumOfPractical+=Weigheddegrees5y4m_5y7m.picturecomplete(CalculationFromTester.PictureComplete);
    print("SumOfPractical${SumOfPractical}");
    SumOfPractical+=Weigheddegrees5y4m_5y7m.objectassrmbly(CalculationFromTester.PictureComplete);
    print("SumOfPractical${SumOfPractical}");
    SumOfPractical+=Weigheddegrees5y4m_5y7m.blockdesign(CalculationFromTester.BlockDesign);
    print("SumOfPractical${SumOfPractical}");
    SumOfPractical+=Weigheddegrees5y4m_5y7m.maze(CalculationFromTester.Maze);
    print("SumOfPractical${SumOfPractical}");
    // print("SumOfPractical${SumOfPractical}");
  ...
  static String Evaluation(){
    ...
    if(Iq_degree>=150){
      evalations='عبقري';
    }else if(Iq_degree>130&&Iq_degree<=150){
      evalations='موهوب';
    }else if(Iq_degree>=110&&Iq_degree<=130){
      evalations='فوق المتوسط';
    }else if(Iq_degree>=90&&Iq_degree<110){
      evalations='متوسط';
    }else {
      evalations='بطئ التعلم';
    }
    return evalations;
  }
  static Map result(){
