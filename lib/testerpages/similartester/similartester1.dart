import 'package:codecansecond/testerpages/similartester/similartester2.dart';
...
class _pg extends State<SimilarTester1Class> {
  ...
  int x = 0;
  int counters = 2;
  var question = "ما هو الشبه بين البرتقال والموز";
  //var x=GlobalKey();
              ...
              if (check == true) {
                if (counters > 12) {
                  Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (context) {
                        return SimilarTester2Class();
                      },
                    ),
                  );
                } else {
                  var questions = await sql.readData(
                      "SELECT question FROM 'questiondata' WHERE department='s' AND id=${counters} ");
                  setState(() {
